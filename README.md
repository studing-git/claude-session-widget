...
- **↻ 버튼**: 데이터 새로고침 (강제 갱신)
- **⬆ 업데이트 버튼**: 항상 표시. 누르면 새 버전을 확인하고 있으면 바로 받아 재시작 (아래 "자동 업데이트" 참고)
- **─ 버튼**: 창 최소화
- **✕ 버튼**: 앱 종료
- **연결하기 버튼**: 평소 쓰는 브라우저에서 사용량 페이지를 열어 확장 프로그램으로 연결
- **전환 버튼**: 계정 전환 — 평소 쓰는 브라우저에서 계정 전환 페이지를 엶 (행에 마우스를 올리면 표시)
...
2. **실행 중일 때**: 앱이 시작 5초 후와 이후 30분마다 `git fetch`로 새 커밋을 확인합니다. 새 버전이 있으면 타이틀바 버튼이 주황색 **⬆ 업데이트** 알약으로 바뀌고, 클릭하면 pull → (필요 시) `npm install` → 자동 재시작합니다.
3. **직접 확인**: 기다리지 않고 타이틀바의 **⬆** 버튼을 누르면 그 자리에서 확인하고, 새 버전이 있으면 바로 받아 재시작합니다. 최신이면 "이미 최신 버전입니다" 라고 알려 줍니다. 터미널에서 `git pull` 할 필요가 없습니다. (버튼에 마우스를 올리면 현재 버전이 보입니다)
...
1. 앱 시작
2. 데이터 로딩 중... 화면 표시
3. 연결 필요 시 "연결하기" 버튼 클릭 (브라우저에서 사용량 페이지가 열립니다)
4. 확장 프로그램이 값·쿠키를 보내면 자동으로 데이터 표시
...
- **계정 전환** — 연결된 행에 마우스를 올리면 `전환` 버튼이 나타납니다. **평소 쓰는 실제 브라우저**에서 그 서비스의 계정 전환 페이지를 엽니다(구글은 계정 선택 화면). 브라우저에서 다른 계정으로 로그인한 뒤 사용량 페이지를 열면 확장이 새 계정 값을 위젯으로 보냅니다.
...
- **미니 모드**: 3사를 한 줄씩 요약. 이름 대신 **브랜드 아이콘**으로 구분하고, 이름·재설정 시각은 hover 시 툴팁으로 확인
...
**작동 방식**: 확장은 두 가지를 위젯의 로컬 서버(`127.0.0.1:47836`)로 넘깁니다.
1. **사용량 값** — 사용량 페이지를 열어 두면 위젯과 같은 파서로 읽어 즉시 전달(그 행에 `확장` 배지).
2. **세션 쿠키** — 그 프로필의 로그인 쿠키를 위젯 세션에 주입합니다. 한 번 주입되면 **브라우저를 닫아도** 위젯이 그 쿠키로 사용량을 직접 조회해 계속 갱신합니다. (v2.1)
...
4. 위젯을 실행해 둔 상태로, Chrome에서 각 서비스 사용량 페이지를 한 번씩 엽니다:
   ...
   페이지를 한 번 열면 확장이 값과 쿠키를 보냅니다. 이후에는 위젯이 주입된 쿠키로 스스로 갱신하며, 확장은 위젯이 켜져 있을 때 30분마다 최신 쿠키만 다시 넘깁니다(예전처럼 배경 탭으로 AI 사이트를 자동으로 열지 않습니다).
...
- **파싱 실패를 명확히 알립니다.** 사용량은 각 서비스의 화면을 읽어(웹 파싱) 얻기 때문에, 서비스가 페이지를 개편하면 깨질 수 있습니다. 예전에는 그게 조용히 넘어갔습니다.
  - **틀린 값을 정상값처럼 보여주던 문제** — Claude 파서가 게이지 값을 못 읽으면 `|| 0` 때문에 **0%로 둔갑**해, 실제로는 읽기에 실패한 것을 "사용량 0%"로 표시했습니다. 이제 못 읽은 항목은 버리고 실패로 드러냅니다. 확장이 보낸 값에도 같은 문제가 있어 함께 고쳤습니다.
  - **로그인 탓으로 돌리던 문제** — 지표를 못 찾으면 무조건 "로그인을 확인해 주세요"라고 해서, 멀쩡히 로그인된 사용자가 파서가 깨진 줄 모르고 재로그인만 반복하게 됐습니다. 이제 **인증 쿠키가 없다는 걸 아는 경우에만** 로그인 문제로 안내합니다.
  - **표시**: 통합 모드는 `읽기 실패`(빨강), 미니 모드는 `읽기 실패`로 보여 주고, 마우스를 올리면 원인 설명과 **받은 화면이 저장된 경로**(`<userData>/debug/<id>.html`)를 알려 줍니다.
  - **일부만 읽힌 경우**도 값 옆에 `일부` 배지로 알립니다 (예: 게이지 3개 중 2개만 해석됨).


### 2026-09-15 (v2.4.0)
- **확장이 자주 끊기던 문제 수정.** 원인이 여러 개였습니다.
  - **주기 기부 알람이 영영 안 울릴 수 있었음** — 서비스 워커 최상위에서 `chrome.alarms.create` 를 무조건 호출해, MV3 워커가 재시작될 때마다(유휴 30초면 종료된다) 알람 주기가 "지금부터 다시"로 리셋됐습니다. 이제 **없을 때만** 만듭니다.
  - **브라우저를 껐다 켜면 갱신 트리거가 없었음** — `chrome.runtime.onStartup` 핸들러를 추가했습니다.
  - **기부 주기 30분 → 5분** — 위젯을 늦게 켰거나 브라우저를 재시작해도 빨리 복구됩니다. 쿠키가 실제로 바뀐 경우에만 위젯이 다시 조회하므로 사이트를 반복 호출하지 않습니다.
  - **위젯을 재시작하면 로그인이 풀리던 문제** — 만료 없는 세션 쿠키를 그대로 주입해 위젯 종료 시 사라졌습니다. 이제 기본 14일 만료를 붙여 디스크에 보관합니다.
  - **멀쩡한데 `확장` 배지가 사라지던 문제** — 배지가 사용량 보고(사용량 페이지에 있을 때만 옴)만 보고 판단했습니다. 이제 쿠키 기부도 연결 신호로 인정하고, 위젯이 갱신할 때마다 상태를 다시 읽습니다.


### 2026-09-12 (v2.3.0)
- **제공자 브랜드 아이콘**: 이름 앞의 단색 점을 각 AI를 나타내는 아이콘으로 교체했습니다. 미니 모드는 **아이콘만** 표시해(이름은 툴팁) 좁은 폭에서 한눈에 구분되고, 통합 모드는 아이콘 + 이름을 함께 보여 줍니다.
- 아이콘은 인라인 SVG로 그려 넣어 외부 파일·네트워크가 필요 없습니다(위젯 CSP가 외부 이미지와 `data:` URI를 막기 때문). 식별용으로 직접 그린 단순 도형이며, 색은 기존 제공자 색을 그대로 씁니다.


### 2026-09-11 (v2.2.0)
- **업데이트 확인 버튼**: 제목줄의 `⬆` 를 누르면 그 자리에서 새 버전을 확인하고, 있으면 **바로 받아 재시작**합니다(터미널에서 `git pull` 할 필요 없음). 예전에는 새 버전이 감지됐을 때만 버튼이 나타나 직접 확인할 방법이 없었습니다.
- 버튼이 **항상 보입니다**. 평소엔 아이콘(`⬆` = 확인), 새 버전이 감지되면 주황 알약(`⬆ 업데이트` = 지금 적용)으로 바뀝니다.
- 버튼 툴팁에 **현재 버전**을 표시해 위젯에서 바로 확인할 수 있습니다.

### 2026-09-11 (v2.1.0)
- **쿠키 기부 → 위젯 단독 조회**: 확장이 `chrome.cookies` 로 그 프로필의 세션 쿠키를 위젯에 넘기면(`POST /cookies`), 위젯이 자체 세션에 주입해 **브라우저를 꺼도** 사용량을 스스로 갱신함. 다중 크롬 프로필 대응(각 프로필에 확장 설치)
- **배경 탭 자동 열기 제거**: 확장이 더 이상 15분마다 AI 사이트를 배경 탭으로 열지 않음. 쿠키 기부는 위젯이 켜져 있을 때 30분 주기(+사용량 페이지 방문 시)
- **ChatGPT 단독 조회 보정**: 위젯 오프스크린 조회에서도 해시 라우트(`#settings/Usage`)를 흔들어 사용량 패널이 뜨게 함
- **확장 권한 정비**: `cookies` 권한과 구글·OpenAI 인증 도메인 host 권한 추가, 미사용 `tabs` 권한 제거
- `cookie-tools.setCookies` / `extension-server` `/cookies` 라우트(토큰·크기 제한 공유) 추가
