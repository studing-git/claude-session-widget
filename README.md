...
### 2026-09-11 (v2.0.1)
- **확장 오류 수정**: 배경 갱신 탭을 닫을 때 나던 `Uncaught (in promise) Error: No tab with id` 해결 — `chrome.tabs.remove` 의 Promise 거부를 처리하고, 탭을 우리가 연 것만 추적해 닫음
- **배경 탭 최소화**: 위젯이 켜져 있을 때만 사용량 페이지를 열고(설치 직후 포함), 값 보고가 오면 15초 기다리지 않고 즉시 닫음
- **계정 전환을 브라우저에서**: `전환` 버튼이 위젯 내장 로그인 창 대신 평소 쓰는 실제 브라우저에서 계정 전환 페이지를 엶(Google은 계정 선택 화면). 사용하지 않게 된 내장 로그인 창 코드 제거
