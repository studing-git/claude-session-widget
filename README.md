...
**제공자마다 쿠키가 완전히 분리되어 있습니다**(Electron 세션 파티션 `persist:<id>`). 따라서 서비스별로 서로 다른 계정을 써도 충돌하지 않고, 한 서비스에서 로그아웃해도 나머지 로그인은 그대로 유지됩니다.

- **기존 로그인 이어받기** — 예전 버전은 세 제공자가 `defaultSession` 을 공유했습니다. 파티션으로 나누면서 그때의 로그인이 끊기므로, 파티션을 처음 쓸 때 `defaultSession` 에 남아 있는 해당 도메인 쿠키를 **한 번만 옮겨옵니다**. 다시 로그인할 필요가 없습니다.
- **로그인** — 연결되지 않은 행의 `로그인` 버튼을 누르면 해당 서비스 로그인 창이 열립니다. 창을 닫으면 자동으로 다시 조회합니다.
- **계정 전환** — 연결된 행에 마우스를 올리면 `전환` 버튼이 나타납니다. 해당 제공자의 쿠키만 비운 뒤 로그인 창을 엽니다. 쿠키를 비우지 않으면 사이트가 기존 로그인을 그대로 인정해 **계정 선택 화면이 나오지 않기 때문**입니다. 실행 전 확인을 받습니다.
