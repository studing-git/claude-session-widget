...
- **브라우저** — 연결되지 않은 행의 `브라우저` 버튼을 누르면 **평소 쓰는 실제 브라우저**에서 그 서비스 페이지를 새 탭으로 엽니다. 이미 로그인돼 있으면 그대로 사용되고, 확장 프로그램이 설치돼 있으면 그 값이 위젯으로 넘어옵니다. (위젯 전용 프로필이 아니라 사용자님의 기본 브라우저입니다.)
