...
- **브라우저** — 연결되지 않은 행의 `브라우저` 버튼을 누르면 **평소 쓰는 실제 브라우저**에서 그 서비스의 **사용량 페이지**를 새 탭으로 엽니다. 이 페이지에서 확장 프로그램이 값을 읽어 위젯으로 보냅니다(확장이 사용량 페이지 주소에서만 동작하므로, 로그인 페이지가 아니라 사용량 페이지를 엽니다). 이미 로그인돼 있으면 그대로 사용되고, 확장 프로그램이 설치돼 있으면 그 값이 위젯으로 넘어옵니다. (위젯 전용 프로필이 아니라 사용자님의 기본 브라우저입니다.)
...
- ChatGPT는 설정 화면이 해시 주소(`#settings/Usage`)라 배경 자동 갱신이 불확실합니다. 값이 오래되면 설정 › 사용량 탭을 한 번 열어 주세요.
...
Chrome 방식인 제공자는 이름 아래에 **`Chrome 조회`** 배지가 붙습니다. **클릭하면 위젯 세션으로 되돌아갑니다.**

> **중요** — 조회 방식과 로그인 위치는 반드시 일치해야 합니다. `Chrome 조회` 상태에서 위젯의 `로그인` 버튼으로 로그인하면, 로그인은 위젯 세션에 저장되는데 조회는 Chrome 프로필에서 하므로 영원히 연결되지 않습니다. 그래서 **위젯 로그인 창에서 로그인에 성공하면 조회 방식을 자동으로 위젯 세션으로 되돌립니다.**
